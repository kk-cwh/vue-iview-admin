<template>
  <div class="layout-sider">
    <slot name="logo"></slot>
    <Menu v-show="!shrink" ref="sideMenu" :active-name="$route.name" :open-names="openNames" :theme="menuTheme" width="auto" @on-select="changeRoute" @on-open-change="changeOpenNames">
    ...
    <template v-for="(item, index) in showMenuList">
      <Dropdown transfer v-if="!item.hidden && item.children.length>=1" placement="right-start" :key="index" @on-click="changeRoute">
        <Button style="width: 50px;padding:10px 0;border:0" type="text">
        ...
        <DropdownMenu style="width: 180px;" slot="list">
          <template v-for="(child, i) in item.children">
            <DropdownItem :name="child.name" :key="i">
              ...
              <span style="padding-left:10px;" v-if="child.meta && child.meta.title" >{{child.meta.title }}</span>
            </DropdownItem>
          </template>
        </DropdownMenu>
      </Dropdown>
