                        ...
                        <FormItem label="" :label-width="1" prop="title" :rules="{ required: true, message: '请选择文章标签', trigger: 'blur' }">
                            ...
                            <Row>
                                ...
                                <Select v-model="tagIds" multiple size="small">
                                    <Option v-for="item in tags" :value="item.id" :key="item.id">{{ item.tag }}</Option>
                                </Select>
                                </Col>
                                <Col span="6">
...
export default {
    ...
    methods: {
        async  allTags() {
            try {
                const res = await this.$store.dispatch("GetAllTags", { per_page: 100, fields: 'select:id|tag' });
                this.tags = res.data
            } catch (error) {
        ...
        async  allCategories() {
            try {
                const res = await this.$store.dispatch("GetAllCategory", { per_page: 100, fields: 'select:id|tag' });
